...
#initial skull strip
	...
	if [[ ! -e $TMPDIR/$subjName/${subjName}_${ss}_T1w_N4corrected_preproc.nii.gz ]] ; then
	...
	#remove things
	#rm ${simg_input_dir}/${subjName}_${ss}_T1w_N4corrected_norm_preproc.nii.gz
	...
	cd $TMPDIR/${subjName}
        echo $PWD
        ${singularity} antsMultivariateTemplateConstruction.sh -d '3' -b '0' -i '2' -k '1' -t SyN -m '50x80x20' -s CC -t GR -n '0' -r '1' -g '0.2' -c '2' -j '12' -o ${subjName}_${ss}_T2w_nlinMoCo_res-iso.3_N4corrected_brain_ ${simg_input_dir}/${subjName}_${ss}_T2w_run-1_res-iso.3_N4corrected_norm_brain_preproc.nii.gz ${simg_input_dir}/${subjName}_${ss}_T2w_run-2_res-iso.3_N4corrected_norm_brain_preproc.nii.gz ${simg_input_dir}/${subjName}_${ss}_T2w_run-3_res-iso.3_N4corrected_norm_brain_preproc.nii.gz 

	if [[ ! -e $TMPDIR/$subjName/${subjName}_${ss}_T2w_nlinMoCo_res-iso.3_N4corrected_brain_template0.nii.gz  ]] ; then
                echo  "${subjName}_${ss} T2w failed NlinMoCo " >> ${data_dir}/preprocessing_error_log.txt
        fi
	$singularity DenoiseImage -d 3 -n Rician -i ${simg_input_dir}/${subjName}_${ss}_T2w_nlinMoCo_res-iso.3_N4corrected_brain_template0.nii.gz -o ${simg_input_dir}/${subjName}_${ss}_T2w_NlinMoCo_res-iso.3_N4corrected_denoised_brain_preproc.nii.gz -v
	...
	rsync -rv $TMPDIR/${subjName}/* ${data_dir}/$subjName
