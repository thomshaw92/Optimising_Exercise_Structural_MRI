...
for ss in ses-01 ses-02 ses-03 ses-04 ses-05 ; do
	...
	for x in "1" "2" "3" ; do
	    if [[ ! -e ${data_dir}/${subjName}_${ss}_T2w_run-${x}_brainmask.nii.gz ]] ; then
	...
	if [[ ! -e ${data_dir}/${subjName}_${ss}_T2w_nlinMoCo_res-iso.3_N4corrected_brain_template0.nii.gz ]] ; then
		cd $TMPDIR/${subjName}
        	echo $PWD
